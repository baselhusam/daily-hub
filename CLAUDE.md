...
| Runtime | Node 22 (Docker), port **9999** |
...
5. **Logo uploads** go to `public/uploads/` via `src/app/actions/upload.ts`
...
```env
DATABASE_URL="postgresql://dailyhub:dailyhub@localhost:5432/dailyhub"
```

Docker app service uses `postgresql://dailyhub:dailyhub@db:5432/dailyhub`.
