...
- Projects follows the design too. A summary strip (active, stalled, shipping within 30 days, open tasks) sits above a sticky toolbar with status filters, a text filter, group-by-status, sort (ship date, status, open tasks, progress, recent activity, name) and a list/cards switch that remembers its choice. The list is a sortable table — project, status, progress, open · done, next milestone, ship — and the cards carry milestones you can tick, a stalled notice, and an inline Add task composer.
- A project's page opens with a hero (avatar, status, ship line, age, last touched, progress), four tiles (open with the week's change, finished, milestones with the next due, focus logged), a completion-rhythm bar chart over 14/30/90 days, the task list with the same inline composer and editor as Today, milestones you can add and delete in place, and a weekly-shape card with average or total per weekday.
- New and Edit project are one sheet: name, description, an avatar box (initials, icon, or image), status, colour (palette, spectrum, or sampled from the logo), and the ship date. Milestones moved out of the form and onto the project page.
- Habits follows the design: four tiles (kept today, average consistency over 12 weeks with the change since last week, the longest running chain with its history, and habits at risk under 50%), then a table of each habit with its schedule pills, a 14-day strip, its consistency, and a chain length — "4-day chain", "Chain broken", or "New". The habit sheet matches the project one: name, a schedule with Every day / Weekdays / Weekends presets, an icon or image avatar, and Active / Paused.
- Analytics is "Looking back" over 7, 30 or 90 days, switched without a refetch. Four headline cards — tasks closed with the weekday shape, habits kept as a fabric of the last week, focus logged around the clock, active days as a calendar — each with the change against the period before. Project rhythm plots closes per day (or cumulative) for everything or one project, with a by-project mode that overlays the busiest lines, a hover readout per day, and a project panel with sparklines. Below it: where the time went, project progress, a 21-day habit consistency grid in each habit's colour, best and worst weekdays, and an hour-of-day profile. The MCP `get_stats` tool now returns a compact 14-day headline.
